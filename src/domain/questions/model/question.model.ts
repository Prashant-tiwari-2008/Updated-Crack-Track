...
import { QuestionSerializar } from "../../../library/serializer/question/question.serializer";
import { QuestionMiddleware } from "../redux/question.middleware";
import { Comment, IQuestionForm, Question } from "./Question";
...
export class QuestionModel extends Model<Question> {
  ...
  static async createQuestion(question: IQuestionForm) {
    const serializedQuestion = QuestionSerializar.for(question).serialize();
    await this.questionCollection.create(
      serializedQuestion
    );
  }
